...
export default class Store implements IStore {
    ...
    private reduce: ReduxReducer = (previousState: ImmutableMap<string, IReducer<any>>, action: Action) => {
        return Object.keys(this.reducerObjects).reduce(
            (nextState: ImmutableMap<string, IReducer<any>>, key: string) => {
                return nextState.updateIn([key], (v) => this.reducerObjects.get(key).reduce(v, action));
            },
            previousState || ImmutableMap(),
        );
    }
}
